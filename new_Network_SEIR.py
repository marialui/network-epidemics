...
def network_creation (path,t):     #t number of nodes
    ...
    for key in nodi:
        for j in range(len(lista_attributi)):
            G.node[int(key)][lista_attributi[j]] = nodi[key][j]  # per ogni attributo della lista, prende il valore dal dizionario
    return(G,diz,p_df)
...
def SEIRmodel(n, f, path):
    ...
    while iterationcounter <= 200:  # for 100 iterations
   ...
   # infezione = plt.show()
    plt.savefig(path+'new_network_SEIR-D')
    plt.clf()
