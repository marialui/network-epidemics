    ...
    for node in ba.nodes():
        for key in cont_dicts:
            ...
            if int(key.split('-')[0])<=len(ba.edges(int(node))) <= int(key.split('-')[1]):
                ...
                break
    # matplotlib histogram of contact distribution
    plt.hist(lista, color='blue', edgecolor='black',
             bins=int(180 / 5))

    # Add labels
    plt.title('Histogram of contact distribution in a erdos_renyi graph')
...
def SEIRmodel(n, f, path):
    ...
    for key in dizionario:
        for j in range(len(lista_attributi)):
            ba.nodes[int(key)][lista_attributi[j]] = dizionario[key][j]  # per ogni attributo della lista, prende il valore dal dizionario
    ...
    while iterationcounter <= 300:  # for 100 iterations
...
if __name__ == "__main__":
    ...
    focolai = 20  # numero di focolai
    path = "/Users/marilu/PycharmProjects/networkepidemics/"
    SEIRmodel(Nodes, focolai, path)
