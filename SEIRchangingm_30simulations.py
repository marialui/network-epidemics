...
def plotnicely(X,num,path):
    ...
    plt.savefig('%s/%s_mean_spreading.png' % (path,w[X]))
    plt.clf()
    ...
    plt.legend(bbox_to_anchor=(0, 1), loc='upper left', ncol=3)
    plt.suptitle("%s curve" %(w[X]))
    plt.savefig('%s/%s_mean_spreading1.png' % (path,w[X]))
    plt.clf()
...
def SEIRmodel(n, m, f,simulations,path):
...
# for 100 iterations (epoche)
 ...
 # let's plot the spreading of the epidemics over time:
    ...
    for comp in COMPARTMENTS:
         data[comp]= plotnicely(comp,simulations,path)
        # this dictionary will be updated in a way that we have
...
def changingm(contacts):

    for m in range(contacts[0], contacts[1]+1,5):
        # i will crete a directory for each value of m in which i will store the corresponding results
        path = '/home/maria/Desktop/network epidemics/M=%s' % (m)
        if not os.path.exists(path):
            os.makedirs(path)

        #i will save the correspondinig dataframe in the corresponding directory

        tabella= SEIRmodel(nodes, m, focolai, k,path)
        tabella.to_csv('%s/RISULTATI_m=%s.csv' % (path, m),index=True, header=True)

    #so for each compartment
    for col in tabella.columns:
    #and for each value of m (number of links) that we are testing:
        for link in range(contacts[0], contacts[1] + 1, 5):
            #we read the conresponding values for that compartment
            tab=pd.read_csv('/home/maria/Desktop/network epidemics/M=%s/RISULTATI_m=%s.csv' % (link, link))
            x=(tab.index.values)
            y=(tab[col].values)
            #we plot all the lines obtained with differents values of m tested for the same compartment
            plt.plot(x, y, label="m= %s" % (link))
            plt.xlabel('Time')
        plt.legend(bbox_to_anchor=(0, 1), loc='upper left', ncol=3)
        plt.suptitle("curves for %s" % (col))
        plt.savefig('/home/maria/Desktop/network epidemics/FINAL_RESULT/RESULT_%s'%(col))
        plt.clf()
...
if __name__ == "__main__":
    ...
    k = 30 #numero di simulazioni
    print(changingm(links))
