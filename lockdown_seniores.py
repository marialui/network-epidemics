...
def SEIRmodel(n, f, path,lockdays):
    ...
    while start_date <= end_date:  # for 100 iterations
        ...
        for person in a['infetti']:  # per ogni spreader (persona infetta)
            if start_date in lockdays and ba.nodes[person]['age']>14:
    ...
    print(datatable)
    datatable.to_csv(r'/Users/marilu/PycharmProjects/networkepidemics/lock_summary/seniores_lockdown_results.csv', index=True, header=True)
    fig = plt.figure()
    plt.xlabel('Time')
    ...
    fig.autofmt_xdate()
    fig.savefig(path+'seniores_lockdown', dpi=fig.dpi)

    plt.axis([datetime.date(2020, 1, 30), date.today(), 0, 0.4])
    plt.savefig(path+'seniores_lockdown(zoomin)')
