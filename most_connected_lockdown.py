...
def network_creation (path,t):     #t number of nodes
   ...
   # dh = nx.degree_histogram(G)
    ...
    q3=np.quantile(degree_sequence, .75)
...
def SEIRmodel(n, f, path,lockdays):
    ...
    while start_date <= end_date:  # for 100 iterations
        ...
        for person in a['infetti']:  # per ogni spreader (persona infetta)
            if start_date in lockdays and person in most_connected:       ###here we introduced a condition to apply the lock down just to the most connected nodes
                    spreadercontacts_= list(ba.neighbors(person))
                    random.shuffle(spreadercontacts_)
                    spreadercontacts=spreadercontacts_[int(len(spreadercontacts_) * .80): int(len(spreadercontacts_) * 1)]
    ...
    fig.autofmt_xdate()
    fig.savefig(path+'lockdown_mostconnected', dpi=fig.dpi)

    plt.axis([datetime.date(2020, 1, 30), date.today(), 0, 0.4])
    plt.savefig(path+'lockdown_mostconnected(zoomin)')
